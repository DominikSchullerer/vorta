#include "renderer_frontend.h"

#include "renderer_backend.h"

#include "core/logger.h"
#include "core/v_memory.h"

// Backend render context
static renderer_backend_t* backend = NULL;

bool8_t renderer_initialize(const char* application_name, struct platform_state_t* platform_state)
{
    backend = v_allocate(sizeof(renderer_backend_t), MEMORY_TAG_RENDERER);

    // TODO: Make this configurable
    renderer_backend_create(RENDERER_BACKEND_TYPE_VULKAN, platform_state, backend);
    backend->frame_number = 0;

    if (!backend->initialize(backend, application_name, platform_state)) {
        V_LOG_FATAL("Renderer backend failed to initialize. Shutting down.");
        return FALSE;
    }

    return TRUE;
}

void renderer_shutdown() 
{
    backend->shutdown(backend);
    v_free(backend, sizeof(renderer_backend_t), MEMORY_TAG_RENDERER);
}


bool8_t renderer_begin_frame(float32_t delta_time)
{
    return backend->begin_frame(backend, delta_time);
}


bool8_t renderer_end_frame(float32_t delta_time) 
{
    bool8_t result = backend->end_frame(backend, delta_time);
    backend->frame_number++;
    return result;
}


bool8_t renderer_draw_frame(render_packet_t* packet)
{
    // If the begin frame returned succesfully, mid-frame operations may continue.
    if (renderer_begin_frame(packet->delta_time)) {

        // End the frame. If this fails, it is likely unrecoverable.
        bool8_t result = renderer_end_frame(packet->delta_time);

        if (!result) {
            V_LOG_ERROR("render_end_frame failed. Shutting down.");
            return FALSE;
        }
    }

    return TRUE;
}
